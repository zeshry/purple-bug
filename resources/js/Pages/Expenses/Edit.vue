<template>
    ...
    >
        <div class="w-1/3 rounded-lg bg-white p-6 shadow-lg">
            <h3 class="mb-4 text-xl font-semibold">Edit Expense Category</h3>
            <form @submit.prevent="submitExpenseCategory">
                <div class="mb-4">
                    ...
                    >
                    <input
                        v-model="expenseCategory.expense_name"
                        type="text"
                        required
...
const emit = defineEmits(['update-expense-category']);
...
const openModal = (expenseCategoryData) => {
    expenseCategory.value = { ...expenseCategoryData };
    isOpen.value = true;
};
...
const submitExpenseCategory = async () => {
    try {
        const response = await axios.post(
            `/update-categories/${expenseCategory.value.id}`, // Updated endpoint for expense category
            expenseCategory.value,
        );

        emit('update-expense-category', response.data);
