<template>
    ...
    >
        <div class="w-1/3 rounded-lg bg-white p-6 shadow-lg">
            <h3 class="mb-4 text-xl font-semibold">Edit Expense</h3>
...
// Expense data model for editing
const expense = ref({
    id: '',
    name: '',
    amount: '',
...
// Submit the updated expense data
const submitExpense = async () => {
    try {
        const response = await axios.post(
            `/update-expense/${expense.value.id}`, // Updated API endpoint
            expense.value, // Include the expense data with the selected category
        );

        emit('update-expense', response.data); // Emit the updated expense data

        showToast('Expense updated successfully!'); // Show success toast
        closeModal(); // Close the modal
    } catch (error) {
        console.error('Error updating expense:', error); // Log any error
    }
};
